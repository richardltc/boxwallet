...
var blockChainInfo models.BTCZBlockchainInfo
var networkInfo models.BTCZNetworkInfo
var ticker models.BTCZTicker
var transactions models.BTCZListTransactions
var walletInfo models.BTCZWalletInfo
var diffGood, diffWarning float64
var lastBCSyncStatus string = ""

func (b BTCZ) About(coinAuth *models.CoinAuth) string {
	var a app.App
	var btcz btczImport.Bitcoinz
	var sCoreVersion string
	info, err := btcz.WalletInfo(coinAuth)
	if err != nil {
	...
	return "  [" + a.Name() + "        v" + a.Version() + "](fg:white)\n" +
		"  [" + btcz.CoinName() + " Core    " + sCoreVersion + "](fg:white)\n\n"
}
...
func (b BTCZ) LiveNetwork() string {
	var bcSynced bool
...
func (b BTCZ) LiveTransactions() (containsZeroConfs bool, rows [][]string) {
	var sRows [][]string
...
func (b BTCZ) RefreshNetwork(coinAuth *models.CoinAuth) {
	...
	blockChainInfo, _ = btcz.BlockchainInfo(coinAuth)
	networkInfo, _ = btcz.NetworkInfo(coinAuth)
	walletInfo, _ = btcz.WalletInfo(coinAuth)
}

func (b BTCZ) RefreshPrice() {
	var btcz btczImport.Bitcoinz

	ticker, _ = btcz.UpdateTickerInfo()
}

func (b BTCZ) RefreshTransactions(coinAuth *models.CoinAuth) {
	var btcz btczImport.Bitcoinz

	transactions, _ = btcz.ListTransactions(coinAuth)
}
